...
def spider_1(url):
    ...
    for title, href, court, add, area, price, danjia, author in zip(titles, hrefs, courts, adds, areas, prices, danjias, authors):
        ...
        try:
            each_response = requests.get(each_href, headers=headers, timeout=40)
        except requests.exceptions.RequestException as e:  # This is the correct syntax
            print (e)
            continue
...
response = requests.get('http://esf.fang.com/', headers=headers, timeout=60)
soup = BeautifulSoup(response.text, 'lxml')
